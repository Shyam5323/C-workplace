...
int main(){
    int var1{35};
    int* p_var2 {&var1};  //pointer
    const int* p_var3 {&var1}; //pointer to consat -- can change the value pointed to but cant change the value of data
    const int* const p_var4 {&var1}; //const pointer to const data -- cant change anything
    int* const p_var5{&var1}; //coint pointer to daata- cant change value pointed to but can change data through dereferincg
